...
vi.mock("svelte", () => ({
    ...
    hasContext: (key) => svelteMock.markerContexts.has(key),
...
describe("createRivetContext", () => {
    ...
    test("supports set/get/has for typed contexts", () => {
        ...
        const rivet = { useActor: vi.fn(), createReactiveActor: vi.fn() };
    ...
    test("reports missing context with a descriptive error", () => {
        const context = createRivetContext("TestRivet");
        expect(() => context.get()).toThrow('Context "TestRivet" not found. Create an app-local Rivet context and call TestRivet.set(...) or TestRivet.setup(...) in a parent layout.');
    });
});
